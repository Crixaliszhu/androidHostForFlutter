...
class Watermark2CameraController(
    ...
    private val onPhotoCaptured: (jpgBytes: ByteArray, jpegOrientation: Int) -> Unit,
    ...
    fun resumePreview() {
        ...
        }.onFailure { notifyError(it.message ?: "回复相机预览失败") }
    }
    ...
    fun takePhoto(): Boolean {
        ...
        val reader = imageRender ?: return false
        return runCatching {
            capturing = true
    ...
    private fun openCamera(viewWidth: Int, viewHeight: Int) {
        ...
        opening = true
        runCatching {
            val cameraId = selectBackCamera() ?: error("设备无可用摄像头")
            ...
            imageRender?.close()
            imageRender = ImageReader.newInstance(
                captureSize.width,
                captureSize.height,
                ImageFormat.JPEG,
                IMAGE_BUFFER_COUNT,
            ).apply {
                setOnImageAvailableListener({ render -> consumeJpeg(render) }, cameraHandler)
            }
    ...
    private fun createCaptureSession() {
        ...
        val readerSurface = imageRender?.surface ?: return
    ...
    private fun cameraExecutor(): Executor {
        return Executor {
            cameraHandler?.post(it) ?: mainHandler.post(it)
        }
    }
    ...
    private fun consumeJpeg(reader: ImageReader) {
        ...
        try {
            ...
            buffer.get(bytes)
            val orientation = jpegOrientation()
            Log.e(TAG, "ImageReader buffer 照片旋转 角度 ===> $orientation")
            captureSession?.stopRepeating()
            mainHandler.post { onPhotoCaptured(bytes, orientation) }
        } catch (t: Throwable) {
            capturing = false
            notifyError(t.message ?: "读取照片数据失败")
        } finally {
            image.close()
        }
    }
    ...
    private fun jpegOrientation(): Int {
        ...
        Log.e(TAG, "照片旋转 角度 ===> $rotation, degree = $deviceDegrees")
    ...
    private fun choosePreviewSize(sizes: Array<out Size>, viewWidth: Int, viewHeight: Int): Size {
        ...
        return sizes.filter { it.width <= MAX_PREVIEW_WIDTH && it.height <= MAX_PREVIEW_HEIGHT }
    ...
    private fun startCameraThread() {
        ...
        cameraThread = HandlerThread("Watermark2Camera").also {
            it.start()
    ...
    private fun hasCameraPermission() =
        PermissionUtils.isGranted(appContext, Manifest.permission.CAMERA)
    ...
    private companion object {
        ...
        const val TAG = "Watermark2CameraController2"
    }
}